...
public class UserController extends AbstractController {
   ...
   /* @RequestMapping(value = "/delete/{userName}" ,method = RequestMethod.GET)
    ...
    public String delUser(@RequestParam("id")int id, Model model) {
        try {
            if (!userService. delById(id)) {
               model.addAttribute("message","操作失败");
            }
            return "redirect:/user/listUser";// 重定向，返回到listUser页面后得把数据再读一遍
        } catch (SSException ee ){
            LogClerk.errLog.error(ee);
            sendErrMsg(ee.getMessage());
            return ADMIN_SYS_ERR_PAGE;
    ...
    public String newUser(User user, @RequestParam("password2")String password2, Model model, RedirectAttributes redirectAttributes) throws SSException {
        try {
            ...
            return "redirect:/user/listUser";                                 /****重定向***/
        } catch (SSException ee ){
            LogClerk.errLog.error(ee);
            sendErrMsg(ee.getMessage());
            return ADMIN_SYS_ERR_PAGE;
